...
**A generic agent's permissions are its own tool's**, not the relay's. The relay does not add or remove them, and cannot inspect an arbitrary generic command's flags in general — but for a small, named set of binaries reachable *only* through the generic adapter today (Antigravity's `agy`), the relay does know that tool's own bypass flag and refuses it the same way, by binary name rather than by adapter.

**What the relay does to protect you**

- Never runs `git push`, and never passes or accepts a permission-bypass flag on a built-in agent's command: Codex's `--dangerously-bypass-approvals-and-sandbox`, `--dangerously-bypass-hook-trust`, and a sandbox of `danger-full-access`; Claude's `--dangerously-skip-permissions` and `--permission-mode bypassPermissions`. The same check also covers Antigravity's `--dangerously-skip-permissions`, checked by its binary name (`agy`) since it has no registered adapter of its own. A configured command containing one is refused by `doctor`, and by `start` even with `--skip-checks`, before anything is launched. All enforced by the test suite. A generic agent running any other, unrecognized binary still cannot be inspected this way — there is no flag vocabulary to check an arbitrary tool against.
